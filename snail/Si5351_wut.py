...
class Si5351_wut(object):
    ...
    def set_PLL(self, pll, target_frequency):
        ...
        if b==0 and (a%2) == 0:
            # if integer division, enabling Integer mode lowers phase noise
            self.i2c.write8(Si5351_PLL_A_CONTROL_REG+pll_value, Si5351_PLL_INTEGER_MODE)
            pass
        ...
        # bit packing into registers, naming convention is crap but is consistent with
        # Silicon Labs AN619 which specifies how to do this

        # value that gets used a couple of times
        intermediate = math.floor(128*b/c)

        p1 = math.floor(128*a + intermediate - 512)
        p2 = math.floor(128*b - c*intermediate)
        p3 = math.floor(intermediate)

        # Blank list with 8 elements, not sure if this is the nicest way to do this next bit, I'm not a Python guy
        tx_buffer = [0]*8
    ...
    def set_freq(self, clock, pll, target_frequency):
        ...
        # clearing the corresponding bit enables the output
        reg &= (1<<clock)
